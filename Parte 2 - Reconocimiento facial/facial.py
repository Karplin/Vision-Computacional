...
def login_capture():
    ...
    if(res_db["affected"]):
        ...
        if img_user in my_files:
            ...
            if comp >= 0.94:
                print("{}La comptabilidad es del {:.1%}{}".format(color_success, float(comp), color_normal))
                printAndShow(screen2, f"Bienvenido, {user_login}", 1)
            else:
                print("{}La comptabilidad es del {:.1%}{}".format(color_error, float(comp), color_normal))
                printAndShow(screen2, "¡Error! Los datos no son compatibles", 0)
            os.remove(img_user)
    
        else:
            printAndShow(screen2, "¡Error! El usuario no fue encontrado", 0)
    else:
        printAndShow(screen2, "¡Error! El usuario no fue encontrado", 0)
    os.remove(img)
...
Label(text="Seleccione una opción", fg=color_white, bg=color_background, font=(font_label, 18), width="500", height="2").pack()
